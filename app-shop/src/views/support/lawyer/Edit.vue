  ...
  <div class="pb50">
    ...
    <el-form size="small" ref="form" :model="form" label-width="100px">
      <div class="basic-setting-content pl16 pr16">
        <!--基本信息-->
        <div class="common-form">编辑律师</div>
        ...
        <el-form-item label="律师账号：" v-if="form.model.shopUser">
          <el-input v-model="form.model.shopUser.user_name" class="max-w460" disabled></el-input>
          <div class="gray9">如需修改请到管理员列表修改</div>
        </el-form-item>
        <el-form-item label="律师密码：" v-if="form.model.shopUser">
          <el-input class="max-w460" disabled></el-input>
          <div class="gray9">如需修改请到管理员列表修改</div>
        </el-form-item>
      </div>
      <!--提交-->
    ...
    methods: {
      ...
      onSubmit: function () {
        ...
        self.$refs.form.validate((valid) => {
          if (valid) {
            self.loading = true;
            LawyerApi.editLawyer(params, true).then(data => {
              self.loading = false;
              self.$message({
                message: '编辑成功',
                type: 'success'
